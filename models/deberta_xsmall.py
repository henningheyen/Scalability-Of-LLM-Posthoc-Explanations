...
classifier = pipeline("zero-shot-classification", model='cross-encoder/nli-deberta-v3-xsmall', use_fast = False)
candidate_labels = candidate_labels=["negative sentiment", "positive sentiment"]

def predict_xsmall(sentences):
...
def get_results_xsmall(sentences):
  return classifier(sentences, candidate_labels)

def get_predictions_xsmall(results):
