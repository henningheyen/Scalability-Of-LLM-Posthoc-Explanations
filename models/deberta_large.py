...
classifier = pipeline("zero-shot-classification", model='cross-encoder/nli-deberta-v3-large', use_fast = False)
candidate_labels = candidate_labels=["negative sentiment", "positive sentiment"]

def predict_large(sentences):
...
def get_results_large(sentences):
  return classifier(sentences, candidate_labels)

def get_predictions_large(results):
